...
local_controller_ip=$(sshpass -p $host_ssh_password ssh $host_username@$host_ip "sshpass -p $controller_password ssh $controller_username@$master_ip 'fuel node | grep controller' | awk '{print \$9}'")
compute_ip=$(sshpass -p $host_ssh_password ssh $host_username@$host_ip "sshpass -p $controller_password ssh $controller_username@$master_ip 'fuel node | grep compute' | awk '{print \$9}'")
